          ...
          <Grid container spacing={2}> 
              ...
              <Controller
                ...
                render={({ ...field }) => (
                  <Autocomplete
                    ...
                    id="combo-box-demo"
                    disabled={
                      editValues?.customerName !== undefined ? true : false
                    }
                    options={customers.map((customer) => {
                      return { id: customer._id, label: customer.customerName };
                    })}
                    sx={{ width: 300 }}
                    onChange={(_event, value) => {
                      setBalance(
                        Number(
                          customers.find(
                            (customer) => customer._id === value?.id
                          )?.balance
                        )
                      );
            ...
            <Grid item xs={12}>
              {balance !== 0 && !Number.isNaN(balance) ? (
                <>
                  Total Balance:{" "}
                  <span style={{ color: "#FF4244", fontWeight: "bold" }}>
                    {" "}
            ...
            <Grid item xs={12}>
              <Controller
                name="date"
                defaultValue={moment(
                  editValues ? editValues?.date : Date.now()
                )}
                rules={{
